...
namespace cryptotest
{
    ...
    class pkcs11
    {
        ...
        public const uint CKS_RW_SO_FUNCTIONS    = 4;
        ...
        public static List<keyfile> keyfiles;
        ...
        public static void DestroyObject(CK_OBJECT_HANDLE hObject)
        {
            if ((rv = destroy_object(session, hObject )) != (int)ReturnValues.CKR_OK)
                throw new Exception("Failed to destroy object - Error: " + Enum.GetName(typeof(ReturnValues), rv));       
        }
        ...
        public static List<keyfile> read_allkeyfiles(string path, string label = null, bool pin_input = false, string pin = null) {
            ...
            try
            {
                ...
                for (uint i = 0; i < slotList.Length; i++)
                {
                    ...
                    if (((session_info.state & CKS_RO_USER_FUNCTIONS) == CKS_RO_USER_FUNCTIONS) ||
                           ((session_info.state & CKS_RW_USER_FUNCTIONS) == CKS_RW_USER_FUNCTIONS))
                    {
                    }
                    else
                    {
                        if ((token_info.flags & CKF_LOGIN_REQUIRED) == CKF_LOGIN_REQUIRED)
                        {
                            ...
                            else
                            {
                                if (pin_input)
                                {
                                    if (pin == null)
                                    {
                                        CkpPromtPinOnlyForm dialog = new CkpPromtPinOnlyForm();
                                        if (UIUtil.ShowDialogAndDestroy(dialog) != DialogResult.OK)
                                            return null;
                                        Login(dialog.pin.ReadString());
                                    }
                                ...
                                else
                                {
                                    ...
                                    if (pin == null)
                                    {
                                        CkpPromtForm dialog = new CkpPromtForm(label, pin_input);
        ...
        public static byte[] pkcs11_read_key(string path, string label)
        {
            try
            {
                keyfiles = read_allkeyfiles(path, label);
        ...
        public static byte[] pkcs11_read_key(string path, string label, string pin)
        {
            try
            {
                keyfiles = read_allkeyfiles(path, label, false, pin);
